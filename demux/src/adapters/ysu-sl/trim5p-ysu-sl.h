 ...
 */
struct Trim5pInfoYsuSl {
  uint insert_start;
  std::optional<MatchInfo> sid_match;
  std::optional<MatchInfo> umi_match;

  Trim5pInfoYsuSl() : insert_start{0}, sid_match{}, umi_match{} {}
};

/**
 * Responsible for trimming 5' adapters from reads with YSU-SL adapters.
 */
class Trim5pYsuSl {
 public:
  Trim5pYsuSl(bool enable_partial, SeqMatcher runway_5p_matcher, SeqMatcher sid_5p_matcher,
  ...
  Trim5pInfoYsuSl Trim(const uint8_t* seq2, size_t length, const char* seq) const;
