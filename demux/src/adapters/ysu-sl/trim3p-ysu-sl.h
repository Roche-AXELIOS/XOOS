 ...
 */
struct Trim3pInfoYsuSl {
  explicit Trim3pInfoYsuSl(uint insert_end);
...
/**
 * Responsible for trimming 3' adapters from reads with YSU-SL adapters.
 */
class Trim3pYsuSl {
 public:
  Trim3pYsuSl(bool enable_partial, SeqMatcher sid_3p_matcher, SeqMatcher umi_sid_spacer_3p_matcher,
  ...
  Trim3pInfoYsuSl Trim(const uint8_t* seq2, size_t seq_length, const char* seq, uint insert_start) const;
