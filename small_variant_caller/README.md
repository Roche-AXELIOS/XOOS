# Small Variant Caller (SVC) User Guide - 0.80.1
...
The `filter_variants` output VCF contains a number of additional or modified fields as part of the germline filtering and re-genotyping process.

| Field         | Definition                                                                                                                                                                                                                                                                                                        |
|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| QUAL          | Variant quality score. The value ranges from 0 to 255. It is calculated as `-10 * log10(1 - P(variant))`, where `P(variant)` is the sum of the prediction probabilities of all non-reference genotypes.                                                                                                           |
...
| PRED_ML       | Prediction probability score for the predicted genotype. The value ranges from 0 to 1. This field is only available to records with filter value `PASS`.                                                                                                                                                          |
| GQ            | Genotype quality. The value ranges from 0 to 255. It is calculated as `-10 * log10(1 - P(genotype))`, where `P(genotype)` is the prediction probability score for the predicted genotype. `GQ` is essentially the Phred score for the value of `PRED_ML`.                                                         |
...
- `0/0`: homozygous reference genotype
...
A variant with the "0/0" genotype classification lacks sufficient evidence to support the variant call. The other three classes are non-reference (i.e. variant) genotypes. A variant's genotype classification is chosen using the highest prediction probability score.
...
##### Creating an ROC curve for the output VCF
...
Here is an ROC curve plotted for `GQ` in SNVs, using the 2025 SBX-D public release data, 30x-downsampled for HG001.

![](docs/assets/snv_roc.svg)
...
| model-germline-sbxd-multisample-snv.txt.gz   | germline | HG002-7 chr1-22 HC (6 samples) | 30x dedup (HG005-7), full run (HG002-4) | SBX-D     | August 2025     |
| model-germline-sbxd-multisample-indel.txt.gz | germline | HG002-7 chr1-22 HC (6 samples) | 30x dedup (HG005-7), full run (HG002-4) | SBX-D     | August 2025     |
...
- Multi-sample pre-trained models have better F1 scores and better support for high depth sequencing data compared to the single-sample pre-trained models, but they require more RAM.
