...
class HighwayConan(ConanFile):
    ...
    def generate(self):
        ...
        tc.variables["HWY_ENABLE_CONTRIB"] = self.options.with_contrib
        tc.generate()
    ...
    def _patch_sources(self):
        ...
        replace_in_file(
            self, cmakelists, "set(CMAKE_POSITION_INDEPENDENT_CODE TRUE)", ""
        )
        replace_in_file(
            self,
            cmakelists,
    ...
    def package_info(self):
        ...
        if "0.15.0" <= Version(self.version) < "1.0.6" or (
            Version(self.version) >= "1.0.6" and self.options.with_test
        ):
            self.cpp_info.components["hwy_test"].set_property(
                "pkg_config_name", "libhwy-test"
            )
